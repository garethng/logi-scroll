...
- 反转设置存在设备易失内存里，深度睡眠后固件可能复位；若重连后仍未恢复，
  可设环境变量 `LOGI_SCROLL_REVERIFY_SECONDS=30` 开启周期性复检补写
  （实验性，默认关闭——周期探测会让部分罗技 BLE 鼠标在使用中断连）
...
- **反转"过段时间就失效"排查**：若闲置后方向复原且需手动 `toggle` 恢复，
  多半是「输入监控」授权失效——日志里反复出现 `0xE00002E2 not permitted` 即是；
  重新编译后必须重新授权一次（见上）
